...
void plotJetEfficiency(const std::string &in_sample, const std::string &out_dir)
{
  ...
  if (in_sample.find("D2") != std::string::npos)

    processing_sample = "D2_";

  if (in_sample.find("D5") != std::string::npos)

    processing_sample = "D5_";

  if (in_sample.find("Compressed_450_445") != std::string::npos)

    processing_sample = "Compressed_450_445_";
  ...
  std::vector<std::string> variable = {"n_pvtx_", "mu_"};
    ...
    for (size_t i = 0; i < region.size(); ++i) {
      for (size_t j = 0; j < suffix.size(); ++j) {
        for (size_t k = 0; k < jvt.size(); ++k) {
          ...
          std::string plot_pref = region[i] + variable[n];
          ...
          for (int i = 3; i < 6; ++i) {
            ...
            leg = new TLegend(.18, .2, .6, .45);

            leg->SetFillColor(0);
            leg->SetBorderSize(0);

            leg->AddEntry(g1, ("N_{JET} #leq " + std::to_string(i + 1) +
                               ", P_{T} = 30 GeV" + jvt_leg).c_str(), "L");
            leg->AddEntry(g2, ("N_{JET} #leq " + std::to_string(i + 1) +
                               ", P_{T} = 40 GeV" + jvt_leg).c_str(), "L");
            leg->AddEntry(g3, ("N_{JET} #leq " + std::to_string(i + 1) +
                               ", P_{T} = 50 GeV" + jvt_leg).c_str(), "L");
            leg->AddEntry(g4, ("N_{JET} #leq " + std::to_string(i + 1) +
                               ", P_{T} = 70 GeV" + jvt_leg).c_str(), "L");
